...
-- MODIFICADO: Adicionado Head Hitbox, Auto-Leave Admin, TriggerBot e Auto-Save
...
local S = {
    ...
    HeadHitbox=false, HeadSize=5, -- NOVO
    TriggerBot=false, -- NOVO
    AutoLeave=false, -- NOVO
    MyHitboxSize=2,
...
-- [[ FUNÇÃO AUTO-SAVE (IDEIA 4) ]]
local fileName = "SysHub_Config.json"
local function SaveConfig()
    if writefile then
        writefile(fileName, HttpService:JSONEncode(S))
    end
end
...
-- [[ LÓGICA ANTI-KICK / AUTO-LEAVE ADMIN ]]
local function checkAdmins()
    if not S.AutoLeave then return end
    for _, p in ipairs(Players:GetPlayers()) do
        if p:GetRankInGroup(0) > 0 or p.AccountAge < 1 then -- Exemplo genérico
             -- Aqui você pode adicionar IDs de admins conhecidos
...
RunService.RenderStepped:Connect(function()
    if S.TriggerBot and Mouse.Target then
        ...
        if model and model:FindFirstChild("Humanoid") and model.Name ~= LP.Name then
            mouse1click() -- Requer executor que suporte mouse1click
        end
    end
end)

-- [[ LOOP DE HITBOX (MODIFICADO PARA CABEÇA SEPARADA) ]]
task.spawn(function()
            ...
            if p ~= LP and p.Character then
                ...
                if root then
                    if S.Hitbox then
                        ...
                        root.Transparency = 1 -- Mantém invisível se o jogo for padrão
                    end
                end

                -- Hitbox Cabeça (SUA IDEIA)
                if head then
                    if S.HeadHitbox then
                        ...
                        head.Size = Vector3.new(1, 1, 1) -- Padrão R6/R15
                        head.Transparency = 0
                    end
                end
            end
...
-- (Mantive as funções de Waypoint e Stepped conforme seu original)

-- [Re-adicione aqui seu código de Waypoints e Loop de Física original para fechar o script]
